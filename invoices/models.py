...
Type of training partner: Training institute"""
    ...
    def save(self, *args, **kwargs):
        ...
        if self.state.lower() == 'uttarakhand':
            # Total GST rate = CGST + SGST
            total_gst_rate = self.cgst_rate + self.sgst_rate
            self.base_amount = self.total_amount / (1 + (total_gst_rate / 100))
            self.cgst_amount = (self.base_amount * self.cgst_rate) / 100
            self.sgst_amount = (self.base_amount * self.sgst_rate) / 100
            self.igst_amount = 0
        else:
            # IGST for other states
            self.base_amount = self.total_amount / (1 + (self.igst_rate / 100))
            self.cgst_amount = 0
            self.sgst_amount = 0
            self.igst_amount = (self.base_amount * self.igst_rate) / 100
        
        # Round off calculation
        subtotal = self.base_amount + self.cgst_amount + self.sgst_amount + self.igst_amount
